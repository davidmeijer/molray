...
and Shade (intersection : Intersection) (scene : Scene) (depth : int) =
    ...
    let naturalColor =
        SCENE_DEFAULT_COLOR + (GetNaturalColor (intersection.Object, position, normal, reflectDirection, scene))
    
    let reflectedColor =
        if depth >= MAX_RAYTRACING_DEPTH then
            Color.Grey
        else
            GetReflectionColor (intersection.Object, position + (0.001 * reflectDirection), reflectDirection, scene, depth)
...
and GetReflectionColor
    ...
    Color.Scale
        (
            object.Surface.Reflect position,
            TraceRay { Start = position; Direction = rayDirection } scene (depth + 1)
        )
...
and GetNaturalColor
    ...
    let addLight
        ...
        let isInShadow =
            ...
            | Some direction ->
                not (direction > Vector.Magnitude lightDirection)
        ...
        if isInShadow then
            ...
            let lightColor =
                if illumination > 0.0 then
                    Color.Scale (illumination, light.Color)
                else
                    SCENE_DEFAULT_COLOR
            ...
            let specularColor =
                if specular > 0.0 then
                    Color.Scale (Math.Pow (specular, object.Surface.Roughness), light.Color)
                else
                    SCENE_DEFAULT_COLOR
...
let GetPoint (x : int) (y : int) (width : int) (height : int) (camera : Camera) =
    let RecenterX x =
        (float x - (float width / 2.0)) / (2.0 * float width)
    
    let RecenterY y =
        - (float y - (float height / 2.0)) / (2.0 * float height)
