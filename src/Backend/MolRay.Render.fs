...
let drawMolecule (molecule : Molecule) =
    let viewPoint = { X = 0.0; Y = 2.0; Z = 30.0 }
    let lookAt = { X = 0.0; Y = 0.0; Z = 0.0 }
    let scenes =
        [| for degree in [| 0.0 |] do // for degree in [| 0.0 .. 1.0 .. 360.0 |] do
            let scene =
                { Name = $"scene_{degree}"
                  Objects =
                    ...
                    |> Array.toList
                  Lights = [ { Position = viewPoint; Color = Color.White } ]
